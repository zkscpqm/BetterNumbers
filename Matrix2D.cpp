...
Matrix2D Matrix2D::identity(unsigned short _l) {
	vector<double> tmpvec(1., _l);
	return diagonal(tmpvec);
}

Matrix2D Matrix2D::diagonal(vector<double> _diag) {
	vector<double> data;
...
vector<double> Matrix2D::getRow(unsigned short row_number) {
	return getRealRow(row_number - 1);
}

vector<double> Matrix2D::getRealRow(unsigned short row_index) {
...
vector<double> Matrix2D::getColumn(unsigned short column_number) {
	return getRealColumn(column_number - 1);

}

vector<double> Matrix2D::getRealColumn(unsigned short column_idx) {
